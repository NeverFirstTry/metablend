...
async function loadWeights(region) {
  const attempts = [
    { cols: 'id, name, score, reports, weight, delta_history', region, hasHistory: true, hasRegion: true },
    { cols: 'id, name, score, reports, weight', region, hasHistory: false, hasRegion: true },
    { cols: 'id, name, score, reports, weight, delta_history', region: null, hasHistory: true, hasRegion: false },
    { cols: 'id, name, score, reports, weight', region: null, hasHistory: false, hasRegion: false },
  ]
...
// Pure core of applyDeltas (unit tested): compute the updated + re-normalized
// rows without touching the database.
//
//   rows:     [{ id, score, reports, delta_history? }]
//   deltaMap: { apiId: delta } or { apiId: [delta, ...] }
//
// Returns { updates, total } or null when no delta matched a known row.
export function buildWeightUpdates(rows, deltaMap, hasHistory) {
  const byId = {}
  ...
  return { updates, total: updates.reduce((s, u) => s + u.rawFactor, 0) }
}
...
export async function applyDeltas(region, deltaMap) {
  ...
  const built = buildWeightUpdates(rows, deltaMap, hasHistory)
  if (!built) return null
  const { updates, total } = built
  const now = new Date().toISOString()

  // Persist as ONE batch upsert (bulk rows need uniform keys, so untouched
  // rows write back their existing delta_history unchanged). The old per-row
  // loop was ~17 round-trips per call and pushed the hourly calibration past
  // its function time limit; it survives below as the fallback for pre-region
  // schemas or a failed batch.
  let batched = false
  if (hasRegion) {
    const existing = {}
    const names = {}
    rows.forEach(r => {
      existing[r.id] = Array.isArray(r.delta_history) ? r.delta_history : []
      names[r.id] = r.name ?? r.id // live DB has NOT NULL on name — the batch INSERT clause must provide it
    })
    const batch = updates.map(u => ({
      ...
      updated_at: now,
      ...(hasHistory ? { delta_history: u.history ?? existing[u.id] } : {}),
    }))
    const { error } = await supabase.from('api_weights').upsert(batch, { onConflict: 'id,region' })
    batched = !error
    // A failing batch silently falls back to ~2× per-row round-trips — that's
    // a performance regression worth knowing about, not just surviving.
    if (error) await logError('weights-batch-upsert', error, { region, rows: batch.length })
  }
