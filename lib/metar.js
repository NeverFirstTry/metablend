...
// Sky decode that refuses to guess. The old path silently dropped
// ceiling-forming layers whose height wasn't reported (VV///, BKN///), which
// let an obscured sky read as green "No ceiling" / VFR. This variant keeps
// the distinction: "no ceiling" vs "height not reported" vs "no sky data".
export function skyInfo(clouds) {
  if (!Array.isArray(clouds)) return { known: false, ceilingFt: null, obscured: false, unknownBase: false }
  const layers = clouds.filter(c => ['BKN', 'OVC', 'OVX', 'VV'].includes(c.cover))
  const bases = layers.map(c => c.base).filter(b => typeof b === 'number')
  return {
    known: true,
    ceilingFt: bases.length ? Math.min(...bases) : null,
    // sky obscured with no reported vertical visibility (VV///, OVX)
    obscured: layers.some(c => (c.cover === 'VV' || c.cover === 'OVX') && typeof c.base !== 'number'),
    // any ceiling-forming layer with no height — category must not claim VFR
    unknownBase: layers.some(c => typeof c.base !== 'number'),
  }
}

// Report-level modifiers worth surfacing instead of leaving them buried in
// the raw text: AUTO (no human observer), COR (corrected), CAVOK.
export function reportFlags(rawOb) {
  if (typeof rawOb !== 'string') return []
  const flags = []
  if (/\bCOR\b/.test(rawOb)) flags.push('COR')
  if (/\bAUTO\b/.test(rawOb)) flags.push('AUTO')
  if (/\bCAVOK\b/.test(rawOb)) flags.push('CAVOK')
  return flags
}

// Which TAF period is valid right now. Base lines (FM/BECMG/initial) are
// superseded by later matching base lines; TEMPO/PROB overlays are returned
// separately because they mean "intermittently", not "instead".
export function activeTafPeriod(fcsts, nowSec) {
  // An unknown reference time yields no period — never period[0] by accident.
  if (!Array.isArray(fcsts) || !Number.isFinite(nowSec)) return { base: null, tempo: null }
  let base = null
  let tempo = null
  for (const f of fcsts) {
    if (!(f.timeFrom <= nowSec && nowSec < f.timeTo)) continue
    const ch = (f.fcstChange ?? '').toUpperCase()
    if (ch.includes('TEMPO') || ch.includes('PROB')) tempo ??= f
    else base = f
  }
  return { base, tempo }
}

export const CAT_RANK = { LIFR: 0, IFR: 1, MVFR: 2, VFR: 3 }

// Live TAF-vs-METAR divergence at category level. Each side supplies
// { visCat, ceilCat, sig: [labels] }; null categories mean "not stated" and
// are never counted as divergence (no guessing). tafOptimistic = reality is
// worse than the forecast said.
export function tafDivergence(metarSide, tafSide) {
  const out = []
  const cmp = (field, m, t) => {
    if (!m || !t || m === t) return
    out.push({ field, metar: m, taf: t, tafOptimistic: CAT_RANK[m] < CAT_RANK[t] })
  }
  cmp('ceiling', metarSide.ceilCat, tafSide.ceilCat)
  cmp('visibility', metarSide.visCat, tafSide.visCat)
  const mSig = new Set(metarSide.sig ?? [])
  const tSig = new Set(tafSide.sig ?? [])
  const onlyM = [...mSig].filter(x => !tSig.has(x))
  const onlyT = [...tSig].filter(x => !mSig.has(x))
  if (onlyM.length) out.push({ field: 'weather', metar: onlyM.join(', '), taf: null, tafOptimistic: true })
  if (onlyT.length) out.push({ field: 'weather', metar: null, taf: onlyT.join(', '), tafOptimistic: false })
  return out
}
