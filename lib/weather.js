...
export async function fetchTomorrow(lat, lon) {
  ...
  try {
    ...
    return {
      ...
      // absent probability must stay null — `?? 0` would publish a fabricated
      // "0 % chance" as if it were a real forecast (see fetchMETNorway)
      rainPct: v.precipitationProbability != null ? Math.round(v.precipitationProbability) : null,
      rainIsProb: v.precipitationProbability != null,
...
export async function fetchMETNorway(lat, lon) {
  ...
  try {
    ...
    // locationforecast reports precipitation AMOUNT; probability_of_precipitation
    // is absent from both compact and complete (verified 2026-08-09 across four
    // continents). The old `?? 0` therefore published a *fabricated* 0 % as a
    // real probability on every single search, dragging the rain consensus
    // toward "no rain" while other sources reported active rain. Amount now
    // becomes a pseudo-probability in the same fallback tier as NASA POWER and
    // GeoSphere: usable when nobody real answered, never a vote of its own.
    const det = next1h?.details ?? next6h?.details
    const pop = det?.probability_of_precipitation
    const mm = det?.precipitation_amount
    return {
      apiId: 'met-norway',
      displayName: 'MET Norway',
      temp: Math.round(instant.air_temperature * 10) / 10,
      feelsLike: Math.round(instant.air_temperature * 10) / 10,
      rainPct: pop ?? (typeof mm === 'number' ? Math.min(Math.round(mm * 50), 90) : null),
      rainIsProb: pop != null,
...
export async function fetchVisualCrossing(lat, lon) {
  ...
  try {
    ...
    return {
      ...
      rainPct:  c.precipprob != null ? Math.round(c.precipprob) : null,
      rainIsProb: c.precipprob != null,
...
// ── 7-day forecast ────────────────────────────────────────────────────────────
// Daily forecast from open-meteo; `model` picks a specific global model
// (default: best match). Returns null on failure — callers decide fallback.
async function fetchOpenMeteoDailyRaw(lat, lon, model) {
  const modelParam = model ? `&models=${model}` : ''
  const url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&daily=temperature_2m_max,temperature_2m_min,precipitation_probability_max,windspeed_10m_max,weathercode,sunrise,sunset&timezone=auto&forecast_days=7${modelParam}`
  try {
    const res = await tfetch(url)
    if (!res.ok) return null
    const data = await res.json()
    ...
    const days = data.daily.time.map((date, i) => ({
      ...
      windKmh:   Math.round(data.daily.windspeed_10m_max?.[i] ?? 0),
      condition: decodeWeatherCode(data.daily.weathercode?.[i]),
      icon:      weatherIcon(data.daily.weathercode?.[i] ?? 100),
    }))
...
// Best-match daily with MET Norway fallback (always resolves to a usable shape).
export async function fetchOpenMeteoForecast(lat, lon) {
  return (await fetchOpenMeteoDailyRaw(lat, lon)) ?? fetchMETForecast(lat, lon)
}

// All independent keyless daily forecasts in parallel, tagged by source id so
// the route can blend them with the same learned weights as the live consensus.
export async function fetchDailyBundle(lat, lon) {
  const [om, gfs, icon, ecmwf, met] = await Promise.all([
    fetchOpenMeteoDailyRaw(lat, lon),
    fetchOpenMeteoDailyRaw(lat, lon, 'gfs_seamless'),
    fetchOpenMeteoDailyRaw(lat, lon, 'icon_seamless'),
    fetchOpenMeteoDailyRaw(lat, lon, 'ecmwf_ifs025'),
    fetchMETForecast(lat, lon),
  ])
  return [
    { apiId: 'open-meteo', ...om },
    { apiId: 'gfs', ...gfs },
    { apiId: 'icon', ...icon },
    { apiId: 'ecmwf', ...ecmwf },
    { apiId: 'met-norway', ...met },
  ].filter(s => s.days?.length)
}

// Blend a daily bundle into one consensus 7-day strip (pure — unit tested).
// Weighted mean per date for the numbers; condition/icon from the
// highest-weighted source that has the date; `sources` = how many reported it.
export function blendDailyForecasts(bundle, weightMap = {}) {
  const sources = (bundle ?? []).filter(b => b?.days?.length)
  if (!sources.length) return { days: [], sunrise: '–', sunset: '–' }

  const byDate = new Map()
  for (const src of sources) {
    const w = weightMap[src.apiId] ?? 0.25
    for (const d of src.days) {
      if (!d?.date) continue
      let e = byDate.get(d.date)
      if (!e) {
        e = { date: d.date, wSum: 0, tMax: 0, tMin: 0, wind: 0, rain: 0, rainW: 0, sources: 0, top: null, topW: -1 }
        byDate.set(d.date, e)
      }
      if (typeof d.tempMax === 'number') {
        e.tMax += d.tempMax * w
        e.tMin += (d.tempMin ?? d.tempMax) * w
        e.wind += (d.windKmh ?? 0) * w
        e.wSum += w
      }
      if (typeof d.rainPct === 'number') { e.rain += d.rainPct * w; e.rainW += w }
      e.sources += 1
      if (w > e.topW) { e.topW = w; e.top = d }
    }
  }
  ...
  const days = [...byDate.values()]
    ...
    .map(e => ({
      date:      e.date,
      tempMax:   Math.round((e.tMax / e.wSum) * 10) / 10,
      tempMin:   Math.round((e.tMin / e.wSum) * 10) / 10,
      rainPct:   e.rainW ? Math.round(e.rain / e.rainW) : null,
      windKmh:   Math.round(e.wind / e.wSum),
      condition: e.top?.condition ?? 'Unknown',
      icon:      e.top?.icon ?? '🌡',
      sources:   e.sources,
    }))

  const withSun = sources.find(s => s.sunrise && s.sunrise !== '–')
  return { days, sunrise: withSun?.sunrise ?? '–', sunset: withSun?.sunset ?? '–' }
}
