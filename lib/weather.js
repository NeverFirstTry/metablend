...
// ── Extra metrics: UV index, air quality, pollen ──────────────────────────────
// Open-Meteo air-quality API is free and keyless. UV comes from the forecast API.
export async function fetchOpenMeteoExtras(lat, lon) {
  const result = { uvIndex: null, aqi: null, pollen: null }

  // UV index (current) from the standard forecast endpoint
  try {
    ...
    if (uvRes.ok) {
      ...
      if (uv !== undefined && uv !== null) result.uvIndex = Math.round(uv * 10) / 10
    }
  } catch { /* ignore */ }

  // AQI + pollen from the air-quality endpoint
  try {
    ...
    if (aqRes.ok) {
      ...
      if (c) {
        if (c.european_aqi !== undefined && c.european_aqi !== null) {
          result.aqi = Math.round(c.european_aqi)
        }
        // Pollen values are grains/m³; sum the available species for a simple index
        const pollenVals = [
          c.alder_pollen, c.birch_pollen, c.grass_pollen,
          c.mugwort_pollen, c.olive_pollen, c.ragweed_pollen,
        ].filter(v => typeof v === 'number')
        if (pollenVals.length) {
          result.pollen = Math.round(pollenVals.reduce((s, v) => s + v, 0))
        }
      }
    }
  } catch { /* ignore */ }
...
// ── Hourly data for "best time of day" ────────────────────────────────────────
// Returns today's remaining daylight hours with temp / rain / wind, plus the
// single best hour for outdoor activities (low rain, comfy temp, low wind).
export async function fetchOpenMeteoHourly(lat, lon) {
  ...
  try {
    ...
    // Only consider remaining daytime hours (current hour … 21:00)
    const candidates = hours.filter(x => x.hour >= nowHour && x.hour >= 7 && x.hour <= 21)
    const pool = candidates.length ? candidates : hours.filter(x => x.hour >= 7 && x.hour <= 21)
    if (!pool.length) return { best: null, hours: [] }

    // Lower score = better. Penalise rain heavily, wind moderately, and
    // distance from the comfortable 15–25 °C band.
    const score = x => {
      const tempPenalty = x.temp < 15 ? (15 - x.temp) : x.temp > 25 ? (x.temp - 25) : 0
      return x.rainPct * 1.0 + x.windKmh * 0.5 + tempPenalty * 2
    }
