...
//  • 'wind'    — wind-speed diffs in km/h against a METAR observation:
//    stricter rewards so wind accuracy nudges rather than dominates.
const SCHEMES = {
  instant: [1, 2, 4, 6],
  daily:   [2, 4, 6, 8],
  wind:    [3, 8, 15, 25],
...
// Exponential moving average used for MetaBlend Local's per-city bias:
// recent errors dominate, old ones fade. First sample starts from 0.
export function emaBias(current, error, alpha = 0.2) {
  const cur = typeof current === 'number' && Number.isFinite(current) ? current : 0
  return Math.round((cur * (1 - alpha) + error * alpha) * 100) / 100
}
