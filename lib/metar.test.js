...
test('activeTafPeriod: later base lines supersede, TEMPO kept separate', () => {
  ...
  // unknown reference time must not silently match the first period
  assert.deepEqual(activeTafPeriod(fcsts, null), { base: null, tempo: null })
  assert.deepEqual(activeTafPeriod(fcsts, undefined), { base: null, tempo: null })
  assert.deepEqual(activeTafPeriod(fcsts, NaN), { base: null, tempo: null })
})
